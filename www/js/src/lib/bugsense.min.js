(function() {
	function detect() {
		return ie ? {
			...
			iphone: iphone,
			ipad: ipad
		}, webkitVersion.test(ua) && (o.version = ua.match(webkitVersion)[1]), o) : android ? {
			name: "Android",
			webkit: t,
			android: t,
			mobile: t,
		...
		}, firefox && (o.name = "Firefox", o.firefox = t), o) : seamonkey ? {
			name: "SeaMonkey",
			seamonkey: t,
	...
	this.bowser = detect(), bowser.msie && bowser.version >= 8 || bowser.chrome && bowser.version >= 10 || bowser.firefox && bowser.version >= 4 || bowser.safari && bowser.version >= 5 || bowser.opera && bowser.version >= 10 ? bowser.a = t : bowser.msie && bowser.version < 8 || bowser.chrome && bowser.version < 10 || bowser.firefox && bowser.version < 4 || bowser.safari && bowser.version < 5 || bowser.opera && bowser.version < 10 ? bowser.c = t : bowser.x = t
}).call(this),
	function(window) {
		...
		TraceKit.noConflict = function() {
				...
				function traceKitWindowOnError(message, url, lineNo) {
					var stack = null;
					if (lastExceptionStack) TraceKit.computeStackTrace.augmentStackTraceWithInitialElement(lastExceptionStack, url, lineNo, message), stack = lastExceptionStack, lastExceptionStack = null, lastException = null;
					else {
						var location = {
							url: url,
							line: lineNo
					...
					return notifyHandlers(stack, "from window.onerror"), _oldOnerrorHandler ? _oldOnerrorHandler.apply(this, arguments) : !1
				}
				...
				function report(ex) {
					...
					if (lastExceptionStack) {
						...
						lastExceptionStack = null, lastException = null, notifyHandlers.apply(null, [s, null].concat(args))
					}
					var stack = TraceKit.computeStackTrace(ex);
					throw lastExceptionStack = stack, lastException = ex, window.setTimeout(function() {
						lastException === ex && (lastExceptionStack = null, lastException = null, notifyHandlers.apply(null, [stack, null].concat(args)))
				...
				function getSource(url) {
					if (!_has(sourceCache, url)) {
						var source = ""; - 1 !== url.indexOf(document.domain) && (source = loadSource(url)), sourceCache[url] = source ? source.split("\n") : []
					}
				...
				function guessFunctionName(url, lineNo) {
					...
					for (var i = 0; maxLines > i; ++i)
						if (line = source[lineNo - i] + line, !_isUndefined(line)) {
				...
				function computeStackTraceFromStackProp(ex) {
					...
					for (var parts, element, chrome = /^\s*at (?:((?:\[object object\])?\S+(?: \[as \S+\])?) )?\(?((?:file|http|https):.*?):(\d+)(?::(\d+))?\)?\s*$/i, gecko = /^\s*(\S*)(?:\((.*?)\))?@((?:file|http|https).*?):(\d+)(?::(\d+))?\s*$/i, lines = ex.stack.split("\n"), stack = [], reference = /^(.*) is undefined$/.exec(ex.message), i = 0, j = lines.length; j > i; ++i) {
						if (parts = gecko.exec(lines[i])) element = {
							...
							if (!(parts = chrome.exec(lines[i]))) continue;
							element = {
								url: parts[2],
								func: parts[1] || UNKNOWN_FUNCTION,
								line: +parts[3],
					...
					return stack[0] && stack[0].line && !stack[0].column && reference && (stack[0].column = findSourceInLine(reference[1], stack[0].url, stack[0].line)), stack.length ? {
						mode: "stack",
						name: ex.name,
						message: ex.message,
						url: document.location.href,
				...
				function computeStackTraceFromOperaMultiLineMessage(ex) {
					...
					var parts, i, len, source, lineRE1 = /^\s*Line (\d+) of linked script ((?:file|http|https)\S+)(?:: in function (\S+))?\s*$/i,
					...
					for (i = 2, len = lines.length; len > i; i += 2) {
						...
						if (item) {
							...
							item.context = context && midline.replace(/^\s*/, "") === lines[i + 1].replace(/^\s*/, "") ? context : [lines[i + 1]], stack.push(item)
						}
					}
					return stack.length ? {
						mode: "multiline",
				...
				var debug = !1,
					sourceCache = {};
				return computeStackTrace.augmentStackTraceWithInitialElement = augmentStackTraceWithInitialElement, computeStackTrace.guessFunctionName = guessFunctionName, computeStackTrace.gatherContext = gatherContext, computeStackTrace.ofCaller = computeStackTraceOfCaller, computeStackTrace
			}(),
			function() {
...
var isObject = function(instance) {
	...
	serialize = function serialize(params, obj, traditional, scope) {
		...
		forEach(obj, function(key, value) {
			scope && (key = traditional ? scope : scope + "[" + (array ? "" : key) + "]"), !scope && array ? params.add(value.name, value.value) : (traditional ? isArray(value) : isObject(value)) ? serialize(params, value, traditional, key) : params.add(key, value)
		})
	},
	param = function(obj, traditional) {
		...
		}, serialize(params, obj, traditional), params.join("&")
	},
	generator = function() {
		var S4 = function() {
...
(function() {
	return this.Lockr = {}, Lockr.set = function(key, value) {
		localStorage.setItem(key, value)
	}, Lockr.hset = function(key, hashObj) {
		localStorage.setItem(key, JSON.stringify(hashObj))
	}, Lockr.get = function(key) {
		var value = localStorage.getItem(key);
		return null == value ? void 0 : value.match(/[\{\}\:\[\]]/) ? JSON.parse(value) : value.match(/^\d+(?:\.\d+$)/) ? parseFloat(value) : value.match(/^\d+$/) ? parseInt(value) : value
...
var md5cycle = function(x, k) {
	...
	md51 = function(s) {
		...
		if (tail[i >> 2] |= 128 << (i % 4 << 3), i > 55)
			for (md5cycle(state, tail), i = 0; 16 > i; i++) tail[i] = 0;
...
if ("5d41402abc4b2a76b9719d911017c592" != md5("hello")) var add32 = function(x, y) {
...
window.Bugsense = function() {
	var config = {
		...
		initAndStartSession = function(options) {
			options.appver && (options.appVersion = options.appver, delete options.appver), config = extend(config, options)
		},
	...
	Bugsense.breadcrumbs = [], Bugsense.extraData = {}, Bugsense.addExtraData = function(key, value) {
		...
		isValidKeyValue(breadcrumb) && (Bugsense.breadcrumbs.length + 1 == 16 && (Bugsense.breadcrumbs = Bugsense.breadcrumbs.slice(1)), Bugsense.breadcrumbs.push(breadcrumb))
	...
	var Cache = {
		...
		cacheReport: function(data) {
			this._queue.push(data), this.update()
		},
		retrieve: function() {
			var data = Lockr.get("bugsense_cache");
			this._queue = data || [], this._queue.length && this.sendCachedReport()
		},
		update: function() {
		...
		sendCachedReport: function() {
			...
			each(this._queue, function(data, index) {
				Bugsense.Network.send(data, "POST"), that._queue.shift(index)
			}), this.update()
		}
	};
	return Cache
	...
	var computeErrorHash = function(offendingLine, message, line, klass, appVersion) {
		...
		parse = function(data) {
			...
			return parsedError.custom_data = data.custom_data || {}, parsedError
		},
		generateExceptionData = function(error) {
			...
			}), crash.application_environment.log_data = extend(Bugsense.extraData, error.custom_data), crash
		},
	...
	return Bugsense.notify = function(data, custom_data) {
		var parsedError = {};
		return custom_data && (data.custom_data = custom_data), parsedError = parse(data), Bugsense.Network.send(generateExceptionData(parsedError), "POST"), !1
	}, window.onerror = function(exception, url, line, column, errorobj) {
		if (!Bugsense.config.disableOnError) {
			if (Bugsense.config.apiKey) return Bugsense.trigger("crash"), Bugsense.notify({
				exception: exception,
				url: url,
				line: line,
				column: column,
			...
			return "warn" in console ? console.warn(msg) : console.log(msg), !1
		}
		return !1
	}, {
		parse: parse,
	...
	var generator = function() {
		...
		Sessions = {
			generateUid: function() {
				...
				return Lockr.set("bugsense:uid", uid), uid
			}
		};
	return Sessions
	...
	var Network = {
		...
		send: function(data, method) {
			...
			console.log(JSON.stringify(data), "bug-log");
			net.open(method, this.getPostURL(), !0), net.setRequestHeader("X-BugSense-Api-Key", Bugsense.config.apiKey), net.setRequestHeader("Content-Type", "application/x-www-form-urlencoded"), net.onerror = function() {
			}, net.onreadystatechange = successHandler, net.send(param({
